...
quickReplyBtn.addEventListener("click", () => {
    const message = quickReplyBtn.textContent.trim();
    if (message) {
        // Display the quick-reply message as outgoing
        displayMessage(message, 'outgoing');
        // Send the message to the bot
        sendMessage(message);
        // Hide the quick-reply button and show the chat input
        quickReplyBtn.style.display = 'none';
        chatInputContainer.style.display = 'flex';
        // Adjust the height of the chatbox
        chatbox.style.height = '470px';
    }
});
